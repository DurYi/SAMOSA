...
def main(args):
    ...
    for i, test_video_path in enumerate(test_video_paths): 
        ...
        visible_path = osp.join(frames_or_path, 'infrared') \
            if not args.inference_on_mp4 else osp.join(frames_or_path, 'infrared.mp4')
        print(f"({i+1}/{len(test_video_paths)}):{visible_path}")
        json_path = osp.join(frames_or_path, 'infrared.json')
