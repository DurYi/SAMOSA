...
from utils.calc_vot_metrics import print_lasotext_results
...
def load_txt_point(gt_path):
    ...
    for line in data:
        ...
        gt_traj.append([x, y, w, h])
        x_center, y_center = (x + w / 2), (y + h / 2)
        x_center, y_center, w, h = int(x_center), int(y_center), int(w), int(h)
        prompts[fid] = ((x, y, x + w, y + h), 0)
        points[fid] = [[x_center, y_center]]
        fid += 1
    return prompts, points, gt_traj

def load_exist(occ_path, ofv_path):
    with open(occ_path, 'r') as f:
        lines = f.readlines()
        if len(lines) == 1:
            occ = lines[0].strip().split(',')
        else:
            occ = [i.strip() for i in lines]
    with open(ofv_path, 'r') as f:
        lines = f.readlines()
        if len(lines) == 1:
            ofv = lines[0].strip().split(',')
        else:
            ofv = [i.strip() for i in lines]
    assert len(occ) == len(ofv), "Occ and Ofv have different lengths!"
    exist = []
    for i in range(len(occ)):
        if occ[i] == '0' and ofv[i] == '0':
            exist.append(1)
        else:
            exist.append(0)
    return exist
...
def main(args):
    ...
    cat_names = os.listdir(args.test_path)
    test_video_names = []
    for cat_name in sorted(cat_names):
        if cat_name.endswith('.txt'):
            continue
        test_video_names.extend(os.listdir(osp.join(args.test_path, cat_name)))
    test_video_names.sort()
    method_name = 'samosa' if 'samosa' in args.mode else args.mode
    out_folder = f"{args.out_root}/{method_name}{args.output_suffix}/LaSOT_ext"
    ...
    for i, test_video_name in enumerate(test_video_names): 
        ...
        cat_name = test_video_name.split('-')[0]
        video_folder = osp.join(args.test_path, cat_name, test_video_name)
        txt_path = osp.join(video_folder , 'groundtruth.txt')
        occ_path = osp.join(video_folder, 'full_occlusion.txt')
        ofv_path = osp.join(video_folder, 'out_of_view.txt')
        frames_path = osp.join(video_folder, "img")
        print(f"({i+1}/{len(test_video_names)}):{frames_path}")
        prompts, points, gt_rect = load_txt_point(txt_path)
        exist = load_exist(occ_path, ofv_path)
        index = 0
        gt_traj = {
            'gt_rect': gt_rect,
            'exist': exist
    ...
    print_lasotext_results(f"{args.out_root}/{method_name}{args.output_suffix}")
    try:
        del predictor, state
    except:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_path", default="data//LaSOT/LaSOT_extension_subset/", help="path to test video path")
