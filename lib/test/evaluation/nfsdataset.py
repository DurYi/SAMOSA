...
class NFSDataset(BaseDataset):
    ...
    def _construct_sequence(self, sequence_info):
        ...
        video = sequence_info["name"].replace("nfs_", "")
        gt_path = f"{self.base_path}/{video}/30/{video}.txt"
        anno_path = f"{self.base_path}/{video}/30/groundtruth.txt"
        
        if not os.path.exists(anno_path):
            print(f"{anno_path} not exists, generate...")
        else:
            os.remove(anno_path)

        f = open(anno_path, "w")
        g = open(gt_path, "r").readlines()
        for idx, line in enumerate(g):
            _, x1, y1, x2, y2, _, _, _, _, _ = line.split(' ')
            if (idx + 1) % 8 == 1:
                x1, y1, x2, y2 = int(x1), int(y1), int(x2), int(y2)
                f.write(f"{x1},{y1},{x2-x1},{y2-y1}\n")
        f.close()
